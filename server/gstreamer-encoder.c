...
static uint64_t get_effective_bit_rate(SpiceGstEncoder *encoder)
{
    ...
    if (encoder->next_frame) {
        ...
        elapsed += MSEC_PER_SEC / get_source_fps(encoder);
    }
    return elapsed ? encoder->stat_sum * 8 * MSEC_PER_SEC / elapsed : 0;
}
...
static void update_next_frame(SpiceGstEncoder *encoder)
{
    ...
    encoder->next_frame = get_last_frame_mm_time(encoder) + delay_ns / NSEC_PER_MILLISEC;
                                          ...
                                          VideoBuffer **outbuf)
{
    ...
    int32_t refill = encoder->bit_rate * elapsed / MSEC_PER_SEC / 8;
