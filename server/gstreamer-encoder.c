...
# define PortableGMutex GMutex*
# define portable_g_mutex_init(mutex) (mutex = g_mutex_new())
# define portable_g_mutex_clear(mutex) g_mutex_free(mutex)
# define portable_g_mutex_lock(mutex) g_mutex_lock(mutex)
# define portable_g_mutex_unlock(mutex) g_mutex_unlock(mutex)
# define PortableGCond GCond*
# define portable_g_cond_init(cond) (cond = g_cond_new())
# define portable_g_cond_clear(cond) g_cond_free (cond)
# define portable_g_cond_signal(cond) g_cond_signal(cond)
# define portable_g_cond_wait(cond, mutex) g_cond_wait(cond, mutex)
#else
# define PortableGMutex GMutex
# define portable_g_mutex_init(mutex) g_mutex_init (&(mutex))
# define portable_g_mutex_clear(mutex) g_mutex_clear (&(mutex))
# define portable_g_mutex_lock(mutex) g_mutex_lock(&(mutex))
# define portable_g_mutex_unlock(mutex) g_mutex_unlock(&(mutex))
# define PortableGCond GCond
# define portable_g_cond_init(cond) g_cond_init (&(cond))
# define portable_g_cond_clear(cond) g_cond_clear (&(cond))
# define portable_g_cond_signal(cond) g_cond_signal(&(cond))
# define portable_g_cond_wait(cond, mutex) g_cond_wait(&(cond), &(mutex))
#endif
...
#   define SPICE_GST_VIDEO_PIPELINE_CAPS     0x4
    ...
    /* Output buffer */
    PortableGMutex outbuf_mutex;
    PortableGCond outbuf_cond;
    VideoBuffer *outbuf;
...
static GstBusSyncReply handle_pipeline_message(GstBus *bus, GstMessage *msg, gpointer video_encoder)
{
    ...
    if (GST_MESSAGE_TYPE(msg) == GST_MESSAGE_ERROR) {
        ...
        portable_g_mutex_lock(encoder->outbuf_mutex);
        encoder->outbuf = (VideoBuffer*)create_gst_video_buffer();
        portable_g_cond_signal(encoder->outbuf_cond);
        portable_g_mutex_unlock(encoder->outbuf_mutex);
    }
    return GST_BUS_PASS;
}
                                  ...
                                  VideoBuffer **outbuf)
{
    ...
    while (!encoder->outbuf) {
        portable_g_cond_wait(encoder->outbuf_cond, encoder->outbuf_mutex);
    }
...
static void spice_gst_encoder_destroy(VideoEncoder *video_encoder)
{
    ...
    free_pipeline(encoder);
    portable_g_mutex_clear(encoder->outbuf_mutex);
    portable_g_cond_clear(encoder->outbuf_cond);
                                    ...
                                    bitmap_unref_t bitmap_unref)
{
    ...
    portable_g_mutex_init(encoder->outbuf_mutex);
    portable_g_cond_init(encoder->outbuf_cond);
